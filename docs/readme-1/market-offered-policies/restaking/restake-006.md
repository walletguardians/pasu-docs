...
# RESTAKE-006: 출금을 토큰이 아닌 지분으로 받을 시 경고

### Policy Definition (정책 정의)

> EigenLayer에서 큐에 넣어둔 출금을 마무리할 때, 토큰이 아니라 지분(shares) 형태로 받으면 한 번 더 확인하도록 경고합니다.

EigenLayer에서 출금할 때에는 토큰으로 받을 수도 있고, 리스테이크 지분으로 받을 수도 있습니다. 리스테이크 지분으로 받으면 출금한 자산이 지갑에 들어오지 않고 지금 위임 중인 오퍼레이터에게 다시 예치·위임됩니다.

이 정책은 출금할 때 토큰이 아니라 지분으로 받게 되는 경우 한 번 더 확인하도록 경고합니다.
