---
description: stETH/wstETH Permit Signature to a Non-Allowlisted Spender (WARN)
---
...
permit은 트랜잭션 없이 서명 한 번으로 토큰 사용 권한을 위임하는 방식입니다. stETH나 wstETH permit에 서명하면 승인 대상은 서명에 담긴 한도만큼 내 stETH·wstETH를 직접 옮길 수 있습니다. 권한을 가져갈 곳이 Permit2나 Lido 출금 큐(Withdrawal Queue)처럼 스테이킹 과정에서 흔히 쓰는 컨트랙트라면 자연스러운 서명이지만, 그 목록에 없는 낯선 주소가 권한을 받아 가려 한다면 토큰을 빼내려는 피싱일 수 있습니다. 따라서 신뢰 목록에 없는 대상에게 stETH·wstETH permit을 서명하려 하면 한도를 주기 전에 승인 대상이 맞는지 확인하도록 경고합니다.

#### Scope (적용 범위)

stETH 또는 wstETH permit 서명에 적용됩니다. (Erc20Permit, 승인 대상이 Permit2·Withdrawal Queue·wstETH 컨트랙트가 아니면서 한도가 0도 무제한도 아닐 때)
...
permit을 서명하는 토큰 주소(`context.token.key.address`), 서명에 담긴 한도(`context.amount`), 권한을 받아 갈 승인 대상(`context.spender`)
...
@severity("warn")
@reason("신뢰 목록 밖의 승인 대상에게 stETH/wstETH permit을 서명합니다 — 한도를 주기 전에 승인 대상을 확인하세요")
forbid (
    principal,
