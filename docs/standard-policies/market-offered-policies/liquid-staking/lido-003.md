...
# LIDO-003: 대량의 stETH를 wstETH로 래핑할 시 경고

### Policy Definition (정책 정의)

> 한 번에 정해진 한도를 넘는 **대량의 stETH**를 wstETH로 래핑하면 경고합니다.
...
Lido에서 stETH를 wstETH로 래핑하는 거래에 적용됩니다. 래핑 금액이 한도(기본 100 stETH)를 넘을 때 경고합니다. (Wrap)
...
@severity("warn")
@reason("한 번에 많은 양의 stETH를 wstETH로 래핑(wrap)하는 대량 거래입니다")
forbid (
    principal,
