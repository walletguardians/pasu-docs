...
> 이번 **대출(Borrow)** 이 실행되면 건강도(health factor)가 1.5 미만으로 내려가는 경우, 청산 위험을 다시 확인하도록 경고합니다.

대출 프로토콜은 담보 대비 빌린 금액의 안전 여유를 건강도(health factor)라는 한 숫자로 보여줍니다. 이 값이 1까지 떨어지면 포지션이 청산되어 담보를 잃습니다. 더 빌릴수록 건강도는 낮아지고, 1에 가까울수록 시세가 조금만 흔들려도 청산으로 넘어갑니다. 그래서 이번 대출을 반영한 건강도를 미리 계산해, 그 값이 1.5 미만이면 빌리는 금액이 적절한지 서명 전에 한 번 더 확인하도록 경고합니다.

#### Scope (적용 범위)

대출 프로토콜에서 자산을 빌리는 트랜잭션. 이번 대출을 반영한 건강도가 1.5 미만으로 떨어지는 경우에 적용됩니다. (Borrow)
...
대출 실행 후의 건강도(`context.custom.postActionHf` — 빌리는 자산(`$.action.asset`)과 금액(`$.action.amount`)을 반영해 서버에서 계산한 결과)
