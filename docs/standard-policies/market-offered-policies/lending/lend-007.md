...
# LEND-007: 예치 금액이 기준을 초과할 시 경고

### Policy Definition (정책 정의)

> 한 번에 예치하는 금액이 기준을 넘는 경우, 규모를 다시 확인하도록 경고합니다.

Aave나 Morpho 같은 대출 프로토콜에 토큰을 예치하면 그만큼 담보가 쌓입니다. 다만 예치 한 번에 큰 금액을 넣는다는 건, 잘못 누른 풀이거나 들어가려던 마켓이 아닐 때 한 번에 묶이는 돈도 그만큼 크다는 뜻입니다. 예치 금액을 현재 시세로 환산한 값이 기준을 넘으면, 풀과 금액이 맞는지 서명 전에 한 번 더 따져보도록 경고합니다.

#### Scope (적용 범위)

대출 프로토콜에 토큰을 예치하는 서명. 예치하는 토큰과 수량을 시세로 환산했을 때 기준을 넘는 경우에 적용됩니다.&#x20;
