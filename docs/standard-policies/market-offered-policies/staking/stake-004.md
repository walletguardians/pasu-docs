# STAKE-004: Aave 스테이킹 자산을 회수할 때 본인 지갑이 아닌 주소로 인출할 시 차단

### Policy Definition (정책 정의)

> Aave 스테이킹 자산을 인출(Redeem)하면서 받는 주소가 **본인 지갑이 아닌** 경우, 인출이 나가지 않도록 **차단**합니다.

Aave Safety Module(stkAAVE)·sGHO·Umbrella에 묶어둔 원금을 다시 찾는 인출(Redeem)을 수행할 때, 자금을 받을 주소를 따로 지정할 수 있습니다. 이 주소가 조작되면 원금이 엉뚱한 곳으로 흘러갈 수 있습니다. dApp이 미리 채워준 주소를 그대로 서명하거나, 받는 주소 칸이 슬쩍 바꿔치기된 피싱에 당하면 원금을 통째로 잃을 수 있습니다. 받는 주소가 인출을 요청한 본인 지갑과 다르면 차단합니다.
