...
그런데 이 스테이킹 함수들은 **지분을 받을 주소를 따로 지정할 수 있습니다. 이 주소가 본인이 아니면 원금은 내 지갑에서 빠져나가지만 그 권리는 남의 주소로 넘어갑니다.**

dApp이 채워준 주소를 그대로 서명하거나 받는 주소 칸이 바꿔치기되면, 스테이킹한 자산을 통째로 잃습니다. 지분을 받을 주소가 스테이킹을 요청한 본인 지갑과 다르면, 경고에 그치지 않고 거래 자체를 막습니다.
