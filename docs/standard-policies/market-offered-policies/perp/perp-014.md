...
Hyperliquid 무기한 선물에서는 시세가 청산가에 닿으면 포지션이 강제로 정리되고 증거금을 잃습니다. 청산가까지 남은 거리가 좁을수록 시세가 조금만 불리하게 움직여도 청산으로 넘어갑니다. 이런 상황에서 포지션을 줄이지 않고 오히려 같은 방향으로 더 키우면, 청산가가 더 바짝 따라붙어 버티는 폭이 거의 사라집니다. 그래서 청산가까지 남은 거리가 10% 미만인 포지션에 리스크를 더하는 주문이면, 정말 지금 키울 자리인지 서명 전에 한 번 더 따져보도록 경고합니다.

#### Scope (적용 범위)

Hyperliquid에 내는 주문 중, 포지션을 줄이는 리듀스온리(reduce-only) 주문이 아니면서 해당 마켓의 기존 포지션이 청산가까지 10% 이내로 가까운 경우에 적용됩니다. (Place Order)

#### Audience (대상 사용자)

Hyperliquid에서 레버리지를 높게 잡고 청산가 근처까지 포지션을 끌고 가는 트레이더

#### Used Data (판정에 사용될 데이터)

주문을 내는 곳이 Hyperliquid인지(`context.venue.name`), 이 주문이 포지션을 줄이는 리듀스온리 주문인지(`context.reduceOnly`), 그리고 해당 마켓 기존 포지션의 청산가까지 남은 거리(`context.liquidationDistanceBps` — 마켓가와 청산가의 차이를 bp로 환산한 값으로, Hyperliquid 계정 조회 결과). 청산가까지 거리가 1,000bp(10%) 미만이면 발동합니다.
