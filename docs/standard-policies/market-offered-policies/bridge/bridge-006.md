...
브리지는 출발 체인에서 자산을 잠그고 도착 체인의 받는 주소로 같은 자산을 다시 풀어주는 식으로 동작합니다. 한 번에 큰 금액을 보낼수록 받는 주소나 도착 체인을 잘못 넣었을 때 손해도 커집니다. 보내는 금액을 현재 시세로 환산한 값이 한도를 넘으면, 정말 이 규모를 옮길지 서명 전에 한 번 더 확인하도록 경고합니다.

#### Scope (적용 범위)

브리지로 자산을 보내는 요청에 적용됩니다. 보내는 자산을 USD로 환산한 값이 한도(기본 $100,000)를 넘을 때 적용됩니다.
...
보내는 토큰과 수량을 USD로 환산한 값(`context.custom.bridgeInputUsd` )이 한도 이하인지 확인합니다.
