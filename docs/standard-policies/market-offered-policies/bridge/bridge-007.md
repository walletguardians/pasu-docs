...
브리지는 출발 체인에서 자산을 잠그고 도착 체인에서 같은 값어치를 풀어주는 방식이라, 보낸 금액과 받는 금액이 거의 같아야 정상입니다. 수수료나 토큰을 바꿔 건너는 경로라면 1~3% 정도는 차이가 날 수 있지만, 그보다 훨씬 크게 벌어진다면 화면에 적힌 받을 금액이 비정상적으로 낮게 잡혔다는 신호입니다. 악성 프론트엔드가 도착 금액을 몰래 깎아 차액을 가로채는 수법이 대표적입니다. 보낸 가치와 받을 가치를 USD로 환산해 비교했을 때 손실이 기준을 넘으면, 서명 전에 도착 체인에서 실제로 받을 토큰과 금액이 맞는지 한 번 더 확인하도록 경고합니다.

#### Scope (적용 범위)

크로스체인 브리지로 자산을 보낼 때(Bridge Send) 적용됩니다. 보낸 입력 가치와 도착 체인에서 받을 출력 가치를 USD로 환산해 줄어든 비율(`context.custom.bridgeValueLossPct`)을 구하고, 이 값이 5%를 넘는 경우입니다.
...
보낸 입력 가치와 받을 출력 가치의 USD 환산 비교로 계산한 손실 비율 (`context.custom.bridgeValueLossPct` — 서버 시세 조회 결과). 계산에는 출발 체인의 자산·금액(`srcToken`·`inputAmount`)과 도착 체인의 자산·금액(`dstChainId`·`dstToken`·`outputAmount`)이 함께 쓰입니다.
...
@severity("warn")
@reason("받는 가치가 보낸 가치보다 크게 적습니다 — 손실이 설정 한도를 넘으니 도착 체인에서 받을 토큰과 금액을 확인하세요")
forbid (
    principal,
