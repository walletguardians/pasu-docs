...
> 한 번의 전송으로 해당 토큰 보유량의 절반을 초과해 옮기는 경우 경고합니다.

가진 토큰의 절반이 넘는 양을 한 번에 옮기는 건 의도한 거래일 수도 있지만, 금액을 잘못 입력했거나 지갑이 털리는 중일 때 흔히 보이는 신호이기도 합니다. 전송량이 보유량의 절반을 넘으면, 정말 이만큼 보낼 것인지 서명 전에 한 번 더 확인하도록 경고합니다.

#### Scope (적용 범위)

전송량이 해당 토큰 보유량의 50%(5,000bp)를 초과하는 경우에 적용됩니다.
...
전송량이 보유량에서 차지하는 비중(`holdingsBp` )이 50%를 넘는지 확인합니다.
