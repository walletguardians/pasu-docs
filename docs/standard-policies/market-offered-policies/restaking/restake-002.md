...
> **본인 지갑이 아닌 다른 주소**로 예치(Deposit) 지분이 잡히는 경우, 서명 전에 내용을 다시 확인하도록 경고합니다.
...
dApp이 잘못된 값을 채웠거나 staker 칸이 공격자 주소로 설정되면, 예치한 자산을 사실상 남에게 넘기게 됩니다. 그래서 지분을 받을 주소가 서명자 본인과 다르면, 의도한 동작이 맞는지 서명 전에 한 번 더 확인하도록 경고합니다.
