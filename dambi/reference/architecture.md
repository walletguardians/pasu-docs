...
DAMBI는 지갑 RPC를 가로채 의미 있는 행동으로 해석하고, 기기 안에서 정책을 평가해 서명 전에 판정을 내립니다. 이 페이지는 그 전체 흐름과 구성 요소를 설명합니다.
...
| **대시보드** | 정책 작성·관리, 마켓플레이스, 시뮬레이션, 히스토리를 담은 React 관리 UI |
| **정책 엔진 (Rust/Cedar)** | `ActionBody` → Cedar lowering과 Cedar 평가. WASM으로 컴파일되어 확장에 내장 |
| **policy-server (Rust/Axum)** | 인증, 지갑·계정 상태, 예측 델타(`/evaluate`), 마켓플레이스 API |
