---
description: 템플릿·정책·패키지·바인딩이 어떻게 연결되는지
---

# 템플릿·정책·패키지의 관계

DAMBI에서 정책을 다루다 보면 **템플릿 / 정책 / 패키지**(그리고 바인딩)라는 말이 나옵니다. 이 페이지는 이 개념들이 서로 어떻게 연결되는지 정리합니다.
...
정책 허브에 올라간 **패키지**(여러 정책 묶음)를 설치하면, 로컬에 **라이브러리 패키지 하나 + 포함된 개별 정책들**로 풀립니다. "허브에 올라간 묶음"과 "내 기기에 풀린 묶음"은 같은 패키지 개념입니다.

> 정책 허브의 게시 단위는 **단일 정책** 또는 **패키지(여러 정책 묶음)** 두 가지입니다.
...
| **템플릿** | 빈칸이 있는 정책/패키지 (설치 시 채워 정책 생성) | 정책 허브 |
| **패키지(pkg)** | 여러 정책을 묶는 단위 (정책 허브 배포 묶음 포함) | 라이브러리(폴더) · 지갑(토글 그룹) |
